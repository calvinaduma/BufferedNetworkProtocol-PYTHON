...
class BufferedTCPEchoServer(object):
    ...
    def start(self):
        ...
        while self.keep_running:
            new_connection, addr = self.tcpServer.accept()
            loop = True
            while (loop):
                try:
                    message = new_connection.recv(self.buffer_size)
                    if len(message):
                        formatString = "!H" + str(len(message)-2) + "s"
                        length = unpack(formatString, message)[0]
                        packedMessage = unpack(formatString, message)[1]
                        payloadBuffer = b""
                        payloadBuffer += packedMessage
                        offset = 2
                        while len(payloadBuffer) < (length-offset):
                            bufferSize = min(1024, length - len(payloadBuffer))
                            try:
                                message = new_connection.recv(bufferSize)
                                if len(message):
                                    formatString = "!" + str(len(message)) + "s"
                                    packedMessage = unpack(formatString, message)[0]
                                    payloadBuffer += packedMessage
                ...
                formatString = "!H" + str(newMessageLength) + "s"
                packedData = pack(formatString, newMessageLength, newMessage)
                try:
                    new_connection.send(packedData)
                except ConnectionResetError as e:
                    break
...
# sends message
def send_message(self,newMessage):
    pass
